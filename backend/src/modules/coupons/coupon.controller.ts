...
export async function getValidCouponDiscount(storeId: string, code: string, subtotal: number, deliveryCharge = 0, db: CouponClient = prisma) {
  const coupon = await db.coupon.findUnique({ where: { storeId_code: { storeId, code: code.toUpperCase() } } });
...
export async function createCoupon(req: Request, res: Response) {
  const data = await prisma.coupon.create({ data: { ...req.body, code: req.body.code.toUpperCase(), storeId: req.params.storeId } });
  return successResponse(res, "Coupon created", serialize(data), 201);
}
export async function updateCoupon(req: Request, res: Response) {
  const data = await prisma.coupon.update({ where: { id: req.params.couponId, storeId: req.params.storeId }, data: req.body });
