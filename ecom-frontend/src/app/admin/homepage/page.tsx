...
import { getAdminHomepageSettings, getAdminTheme, HomepageSettings } from "@/lib/api";
...
export default async function AdminHomepagePage() {
  const [homepage, theme] = await Promise.all([getAdminHomepageSettings(), getAdminTheme()]);

  return (
    <>
      <PageHeader title="Homepage editor" description="Edit storefront homepage text, images, links, and button colors from one production data source." />
      <HomepageEditorForm initialHomepage={homepage ?? fallbackHomepage} initialTheme={theme} />
    </>
  );
}
